...
/**
 * Builds the full aggregate pipeline for a rubric result,
 * joining rubric (with its program_outcomes and levels) and
 * student_assessment (with student, block, and assessment).
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Verify rubric exists and belongs to the same assessment
    const rubric = await db
      .collection('rubrics')
      .findOne({ _id: new ObjectId(rubric_id) });

    if (!rubric) {
      return NextResponse.json({ error: 'Rubric not found' }, { status: 404 });
    }

    if (rubric.assessment_id.toString() !== studentAssessment.assessment_id.toString()) {
      return NextResponse.json(
        { error: 'Rubric does not belong to the assessment of this submission' },
        { status: 400 }
      );
    }
    ...
    if (existing) {
      return NextResponse.json(
        {
          error: 'A result for this rubric already exists. Use PUT to update it.',
...
//   ?rubric_id=...               → all results for a rubric + level distribution
// ─────────────────────────────────────────────────────────────
export async function GET(req: NextRequest) {
  try {
    ...
    if (student_assessment_id) {
      ...
      const results = await rubricResults
        .aggregate([
          ...
          // Join rubric with its program_outcomes, levels, and order
          {
            $lookup: {
              from: 'rubrics',
              localField: 'rubric_id',
    ...
    if (rubric_id) {
      ...
      const results = await rubricResults
        .aggregate(buildAggregatePipeline({ rubric_id: new ObjectId(rubric_id) }))
        .toArray();
